                ...
                <a-list item-layout="horizontal" :data-source="deptdata" ref="editTable">
                  <a-list-item slot="renderItem" slot-scope="item">
                    ...
                    <template slot="actions">
                      ...
                      <a-icon type="close-circle" @click="deletSelectDeptByParentIds(item.id,item.parentIds)"/>
                    </template>
                  </a-list-item>
                </a-list>
              </a-checkbox-group>
    ...
    methods: {
      ...
      onLoadData (treeNode) {
        ...
        return new Promise(resolve => {
          ...
          listDeptTree(treeNode.dataRef.id, 1).then(response => {
             ...
             that.expandSonData = response.data
      ...
      setSelectDeptInfo (checkedNodes) {
        ...
        checkedNodes.forEach(node => {
          ...
          this.setSelectedDeptInfo(id, name, parentIds, deptType)
        })
      },
      ...
      setSelectedDeptInfo (id, name, parentIds, deptType) {
      ...
      deletSelectDeptByParentIds (id, parentIds) {
        // 右侧已选用户悬浮删除按钮删除方法
        if (this.checkedKeys.checked !== undefined) {
          this.checkedKeys.checked = this.checkedKeys.checked.filter(function (item) {
            return id !== item
