package com.aidex.shadd.factory.controller;
...
public class TFactoryController extends BaseController
{
    ...
    public R<TFactory> detail(@PathVariable("id") String id)
    {
        ...
        TFactory tFactory = tFactoryService.findTFactoryById(id);
...
//            data.setFactoryid(tFactory.getId());
...
//        });
    ...
    {
        tFactoryService.modifyTFactory(tFactory);
    ...
    public R<PageInfo>  factoryListByname(TFactory tFactory,HttpServletRequest request, HttpServletResponse response) {
        tFactory.setPage(new PageDomain(request, response));
        return R.data(tFactoryService.findListByName(tFactory));
    }
}
